...
lemlib::Pose odometryPose(0, 0, 0); // the pose of the robot
lemlib::Pose odometrySpeed(0, 0, 0); // the speed of the robot
lemlib::Pose odometryLocalSpeed(0, 0, 0); // the local speed of the robot
...
lemlib::Pose lemlib::getPose(bool radians) {
    if (radians) return odometryPose;
    else return lemlib::Pose(odometryPose.x, odometryPose.y, radToDeg(odometryPose.theta));
}

void lemlib::setPose(lemlib::Pose pose, bool radians) {
    if (radians) odometryPose = pose;
    else odometryPose = lemlib::Pose(pose.x, pose.y, degToRad(pose.theta));
}

lemlib::Pose lemlib::getSpeed(bool radians) {
    if (radians) return odometrySpeed;
    else return lemlib::Pose(odometrySpeed.x, odometrySpeed.y, radToDeg(odometrySpeed.theta));
}

lemlib::Pose lemlib::getLocalSpeed(bool radians) {
    if (radians) return odometryLocalSpeed;
    else return lemlib::Pose(odometryLocalSpeed.x, odometryLocalSpeed.y, radToDeg(odometryLocalSpeed.theta));
}
...
void lemlib::update() {
    ...
    // 3. Inertial Sensor
    // 4. Drivetrain
    float heading = odometryPose.theta;
    ...
    // else, use the the substituted tracking wheels
        ...
        heading -= (deltaVertical1 - deltaVertical2) /
                   (odomSensors.vertical1->getOffset() - odomSensors.vertical2->getOffset());
    float deltaHeading = heading - odometryPose.theta;
    float avgHeading = odometryPose.theta + deltaHeading / 2;
    ...
    // save previous pose
    lemlib::Pose prevPose = odometryPose;

    // calculate global x and y
    odometryPose.x += localY * sin(avgHeading);
    odometryPose.y += localY * cos(avgHeading);
    odometryPose.x += localX * -cos(avgHeading);
    odometryPose.y += localX * sin(avgHeading);
    odometryPose.theta = heading;

    // calculate speed
    odometrySpeed.x = ema((odometryPose.x - prevPose.x) / 0.01, odometrySpeed.x, 0.95);
    odometrySpeed.y = ema((odometryPose.y - prevPose.y) / 0.01, odometrySpeed.y, 0.95);
    odometrySpeed.theta = ema((odometryPose.theta - prevPose.theta) / 0.01, odometrySpeed.theta, 0.95);

    // calculate local speed
    odometryLocalSpeed.x = ema(localX / 0.01, odometryLocalSpeed.x, 0.95);
    odometryLocalSpeed.y = ema(localY / 0.01, odometryLocalSpeed.y, 0.95);
    odometryLocalSpeed.theta = ema(deltaHeading / 0.01, odometryLocalSpeed.theta, 0.95);
}
